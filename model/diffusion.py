...
class GaussianDiffusion(nn.Module):
    ...
    @torch.no_grad() 
    def ddim_sample(self, shape, cond, x_pre = None, **kwargs):
        ...
        if x_pre is not None:
            _, seq, c = x_pre.shape
            x_pre_noised = self.q_sample(x_start=x_pre, t = torch.randint(0, self.n_timestep, (batch,), device=x.device).long(), noise=x)
            x = x.reshape(-1, 150, seq//150, c)
            x_pre_noised = x_pre_noised.reshape(1, 150, seq//150, c)
            x[:,:75] = x_pre_noised[:, 75:]
            x = x.reshape(-1, seq, c)
        ...
        for time, time_next in tqdm(time_pairs, desc = 'sampling loop time step'):
            ...
            if x_pre is not None: 
                _, seq, c = x.shape
                x_pre = x_pre.reshape(-1, 150, seq//150, c)
                x = x.reshape(-1, 150, seq//150, c)
                x[:,:75] = x_pre[:, 75:]
                x = x.reshape(-1, seq, c)
    ...
    @torch.no_grad()
    def lgds_ddim_sample(self, shape, cond, x_0=None, **kwargs):
        """
        Perform long DDIM sampling by iteratively generating samples for each batch instance.

        Args:
            shape (tuple): The shape of the output tensor (batch_size, sequence_length, channels).
            cond (Tensor): The conditioning input tensor.
            x_0 (Tensor, optional): The initial input for the first sequence, if provided.
            **kwargs: Additional arguments.

        Returns:
            Tensor: The generated samples concatenated across the batch.
        """
        batch, device, total_timesteps, sampling_timesteps, eta = (
            shape[0], self.betas.device, self.n_timestep, 50, 1
        )

        # If batch size is 1, directly use standard DDIM sampling
        if batch == 1:
            return self.ddim_sample(shape, cond)

        x_prev = None  # Store the previous sequence for conditioning
        x = torch.tensor([])  # Placeholder for storing generated sequences

        # Iterate through each batch instance for sequential generation
        for i in tqdm(range(batch), desc='Processing batch slices'):
            if i == 0:
                # Initialize the first sequence with given x_0 or a new DDIM sample
                x_prev = x_0 if x_0 is not None else self.ddim_sample((1, shape[1], shape[2]), cond[i:i+1])
            else:
                # Generate the next sequence with previous one as input
                x_current = self.ddim_sample((1, shape[1], shape[2]), cond[i:i+1], x_prev)
                x = torch.cat([x, x_current.cpu()], dim=0)  # Store the result
                x_prev = x_current  # Update x_prev for the next iteration
    ...
    def p_losses(self, x_start, cond, t): 
        ...
        x_start = x_start.permute(0,2,1,3).reshape(bs,sq*dancer_num, c)
        noise = torch.randn_like(x_start)
        ...
        model_out = model_out.reshape(bs, sq, dancer_num, c) 
        target = target.reshape(bs, sq, dancer_num, c)
        model_contact, model_out = torch.split(
            model_out, (4, model_out.shape[3] - 4), dim=3 
        )
        target_contact, target = torch.split(target, (4, target.shape[3] - 4), dim=3) 

        # velocity loss 
        target_v = target[:, 1:, :, 3:] - target[:, :-1, :, 3:] 
        model_out_v = model_out[:, 1:, :, 3:] - model_out[:, :-1, :, 3:]
        ...
        target_dis = target[:,1:,:,:2] - target[:,:-1,:,:2]
        recon_dis = model_out[:,1:,:,:2] - model_out[:,:-1,:,:2]
        dis_loss = self.loss_fn(target_dis, recon_dis, reduction="none")
        ...
        # offset -> xyz
        # model_xyz = offset2xyz(model_x) 
        # target_xyz = offset2xyz(target_x)
        model_x = model_x.reshape(b, s*dancer_num, -1)
        target_x = target_x.reshape(b, s*dancer_num, -1)
        # model_x = offset2xyz(model_x[:,:,:,:2]) 
        # target_x = offset2xyz(target_x[:,:,:,:2]) 
        ...
        # reshape (bs, sq*dancer_num(3), 24, 3) -> (bs, sq, dancer_num(3), 24, 3) | need to reshape otherwise will count v inter different dancers
    ...
    def render_sample( 
        ...
        if isinstance(shape, tuple):
            if mode == "inpaint": 
                ...
                func_class = self.lgds_ddim_sample
            else:
        ...
        samples = samples.reshape(b, 150, s//150, 151) 
        b, s, ds, c = samples.shape 

        if len(samples.shape) == 4 and samples.shape[3] == 151: 
        ...
        poses = np.transpose(poses,(0,2,1,3,4)) 
        ...
        p_map(inner, enumerate(poses)) # render here
